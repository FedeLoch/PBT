...
def process_directory(directory_path, color, label):
    ...
    for file_path in csv_files:
        try:
            ...
            # Col 2: Performance (segunda columna)
            performance_times = pd.to_numeric(df.iloc[:, 1], errors='coerce').fillna(0).values
            
            cum_max_discovery = np.maximum.accumulate(performance_times)
...
def main():
    ...
    plt.title('Heat map comparison - Improvement (Best case as far)')
    plt.xlabel('Iteration (Case)')
    plt.ylabel('Improvement (Best found as far)')
